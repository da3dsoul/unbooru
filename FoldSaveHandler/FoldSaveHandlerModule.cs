...
{
    public class FoldSaveHandlerModule : IModule
    {
        private ISettingsProvider<FoldSaveHandlerSettings> SettingsProvider { get; set; }
        private readonly ILogger<FoldSaveHandlerModule> _logger;
        
        public FoldSaveHandlerModule(ISettingsProvider<FoldSaveHandlerSettings> settingsProvider, ILogger<FoldSaveHandlerModule> logger)
        {
        ...
        private string GetImagePath(string originalName, string title, double aspectRatio)
        {
            ...
            {
                var subpath = aspectRatio is >= 0.3 and <= 0.5 ? "Cover" : aspectRatio is >= 0.7 and <= 1.0 ? "Main" : null;
